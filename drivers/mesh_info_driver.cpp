...
int main(int argc, char *argv[])
{
	...
	// Was using the below did not work.
	...
	GridFunction displacement(fespace), shear(fespace);
	shear = 0.0;
	...
	shear.ProjectCoefficient(ripple); // Project coefficient onto FE space

	// const int SelectDOF = 10;
	// CoordinatesOfDoF(*mesh, SelectDOF, "X", "Vec");
	*nodes += shear; // 6. Deform the mesh: mesh nodes += displacement
